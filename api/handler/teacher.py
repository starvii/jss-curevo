...
from model.entity import Teacher
...
class TeacherAPI(BaseHandler):
    ...
    def query_entity_by_id(self, tid):
        t = self.db.query(Teacher).filter_by(Id=tid).first()
        if t:
    ...
    def query_entities_by_keyword(self, page, size, keyword):
        k = keyword.strip()
        f = Teacher.DeleteAt.is_(None)
        if len(k) > 0:
            k = ''.join(('%', k, '%'))
            o = or_(Teacher.Name.like(k), Teacher.Code.like(k), Teacher.Unit.like(k), Teacher.Dept.like(k),
                    Teacher.Phone.like(k), Teacher.Email.like(k), Teacher.Comment.like(k))
            c = self.db.query(Teacher.DbId).filter(f).filter(o).count()
            l = self.db.query(Teacher).filter(f).filter(o).offset((page - 1) * size).limit(size).all()
        else:
            c = self.db.query(Teacher.DbId).filter(f).count()
            l = self.db.query(Teacher).filter(f).offset((page - 1) * size).limit(size).all()
    ...
    def update_entity(self, entity_dict):
        ...
        try:
            ...
            eid = entity_dict['Id']
            db_id = self.db.query(Teacher.DbId).filter(Teacher.DeleteAt.is_(None), Teacher.Id == eid).scalar()
            if not db_id:
                raise LookupError
            t.DbId = db_id
            self.db.merge(t)
    ...
    def delete_entity(self, eid):
        try:
            result = self.db.execute('UPDATE teacher SET delete_at = :now WHERE delete_at IS NULL AND id = :id',
                                     {'now': now(), 'id': eid})
            self.db.commit()
